...
public class PlayerBoat : MonoBehaviour
{
    ...
    private void Update()
    {
        if (isStarted)
        {
            ...
            // 이부분 때문에 바로 터지는 듯
            if (!isDying && boat.transform.position.x >= 440.4f)
            {
                isDying = true;
    ...
    private IEnumerator Exploding()
    {
        ...
        GameManager.Instance.GetPlayer().transform.parent = thirdPart.transform;
    ...
    {
        if (GameManager.Instance.IsPlayer(col.gameObject) && !isStarted)
        {
            isStarted = true;
            GameManager.Instance.GetPlayer().transform.parent = boat.transform;
        }
        else if (col.collider.CompareTag("Enemy"))
        {
            col.collider.gameObject.transform.parent = boat.transform;
        }
    }
}
